...
public class DefaultListableBeanFactory extends AbstractAutowireCapableBeanFactory
	 ...
	 * 在之后的内容你可能会频繁的见到 “MergedBeanDefinition” 这个词，因此这边先稍微讲解一下，有助于你更好的理解。
	 ...
	 在我们日常使用的过程中，通常会是上面的第3种情况。如果我们使用 XML 配置来注册 bean，则该 bean 定义会被封装成：GenericBeanDefinition；
	    如果我们使用注解的方式来注册 bean，也就是<context:component-scan /> + @Component，则该 bean 定义会被封装成 ScannedGenericBeanDefinition。
	 */
	@Override
		...
		for (String beanName : beanNames) {
			...
			if (!bd.isAbstract() && bd.isSingleton() && !bd.isLazyInit()) {
				...
				if (isFactoryBean(beanName)) {
					...
					// todo 重要 如果BeanName对应的Bean实例不是FactoryBean，则通过BeanName去获取Bean实例. 重要
					getBean(beanName);
				}
			}
		}
	...
	public Object doResolveDependency(DependencyDescriptor descriptor, @Nullable String beanName,
		...
		try {
			...
			if (instanceCandidate instanceof Class) {
				// 调用DependencyDescriptor的resolveCandidate方法. 里面是创建 bean 的入口 重要
